...
export function buildSystemInstruction(context: SceneContext, grounded: boolean): string {
  ...
  lines.push(
    ...
    'Several subjects across several metrics at once: show_chart heatmap.',
    '',
    '# Observed against expected',
    'This is the reasoning frame to reach for whenever a number could have been luck. It comes from the expected-goals family - xG, xA, xGA, xT - and it generalises to everything.',
    'A raw figure says what happened. The same figure against what should have happened says whether it was earned or borrowed, and the GAP is the finding. Nine goals on four expected is a finishing run that will regress, not a striker who has improved. Four on nine is a striker being failed by his luck, not by his movement. The goal count alone cannot tell those apart; the gap always can.',
    'Pass the expectation as matrix.expected on a heatmap and the panel colours the distance from it, warm above and cool below. Then say which way the gap points and what it implies NEXT, because that is the only part the user cannot read off the panel themselves.',
    'It applies far outside football. Views against what a channel of that size normally gets. Engagement against the median for the format. Revenue against the seasonal pattern. Any time you are about to call something good or bad, ask what would have been expected of it first - a figure with no expectation attached is a number, not a finding.',
    'Where the expectation is modelled rather than measured, say so and say whose model. For football that means the analytics accounts on X and sites such as PopFoot, FBref, Understat or Opta - name the one you took it from. An expected value with no provenance is worse than none, because it looks rigorous.',
    'Never invent an expected value to make a gap appear. If you cannot find one, compare against a peer median instead and label it as such.',
    ...
    'research_subject returns the photograph AND the structured facts in one call, fetched from Wikidata and Wikipedia. Call it for any subject you do not have live module data for - a person, a club, a place, a company, a product, a work. Fill the profile from what it returns rather than from memory: a club is one transfer window from being wrong and an age drifts every year.',
    'Wikidata is thin on anything current. For form, recent results, market value, injuries, tactical detail or this season\'s numbers, SEARCH - and search the places that actually cover it. For football that means Transfermarkt, L\'Equipe, Foot Mercato, Maxifoot and Opta; for other fields, the equivalent specialist sources rather than a general encyclopedia. Name the source you took a figure from, in "source", alongside Wikidata.',
    'Keep the two apart. What research_subject returned is the record. What you searched, inferred or judged is your reading, and it belongs in "strengths" and "weaknesses" - which the panel draws below a dividing rule for exactly that reason. Never move an opinion up into the fact list.',
    'Give the weaknesses as honestly as the strengths. A profile carrying only strong points is advertising, and the user can tell.',
    'If research_subject finds nothing, say so. Do not fill the panel from memory to avoid an empty one.',
    '',
